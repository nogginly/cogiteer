...
# site.
private def deployment_with(*lines : String) : Cogiteer::Deployment
  yaml = String.build do |io|
    io << "servers:\n  ollama:\n    protocol: chat_completions\n    url: http://localhost:11434\n"
    io << "deployments:\n  qwen:\n    server: ollama\n    model: qwen3.8\n"
    lines.each { |line| io << "    " << line << "\n" }
  end
  Cogiteer::Config.from_yaml(yaml).deployment("qwen")
end

describe Cogiteer::Config do
  describe "defaults" do
    base = "servers:\n  ollama:\n    protocol: chat_completions\n    url: http://localhost:11434\n" \
           "deployments:\n  qwen:\n    server: ollama\n    model: qwen3.8\n"

    it "is quiet and unstreamed when the block is absent" do
      defaults = Cogiteer::Config.from_yaml(base).defaults
    ...
    it "reads both" do
      config = Cogiteer::Config.from_yaml(
        base + "defaults:\n  streaming: true\n  show_reasoning: true\n")
    ...
    it "refuses a value that is not a boolean, and names the key" do
      expect_raises(Cogiteer::ConfigError, /defaults.streaming/) do
        Cogiteer::Config.from_yaml(base + "defaults:\n  streaming: 1\n")
      end
    end

    it "refuses a defaults that is not a block" do
      expect_raises(Cogiteer::ConfigError, /not a block/) do
  ...
  describe ".from_yaml" do
    it "parses a server and a deployment pointing at it" do
      config = Cogiteer::Config.from_yaml(<<-YAML)
        servers:
          anthropic:
            protocol: anthropic
    ...
    it "raises at parse time for an azure block with no api_version" do
      expect_raises(Cogiteer::ConfigError, /api_version/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            azure-alpha:
              protocol: chat_completions
    ...
    it "raises when 'servers' is missing entirely" do
      expect_raises(Cogiteer::ConfigError, /servers/) do
        Cogiteer::Config.from_yaml("default_deployment: anthropic")
      end
    end

    it "raises when 'deployments' is missing entirely" do
      expect_raises(Cogiteer::ConfigError, /deployments/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            ollama:
              protocol: chat_completions
    ...
    it "recognises the old flat format and says what to change" do
      expect_raises(Cogiteer::ConfigError, /format changed.*servers:/m) do
        Cogiteer::Config.from_yaml(<<-YAML)
          deployments:
            anthropic:
              protocol: anthropic
    ...
    it "raises naming the deployment when its server is not defined" do
      expect_raises(Cogiteer::ConfigError, /"qwen".*"typo-ollama".*not defined/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            ollama:
              protocol: chat_completions
    ...
    it "raises naming the server when 'protocol' is missing" do
      expect_raises(Cogiteer::ConfigError, /"anthropic".*protocol/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            anthropic:
              url: https://api.anthropic.com
    ...
    it "raises naming the server when 'url' is missing" do
      expect_raises(Cogiteer::ConfigError, /"anthropic".*url/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            anthropic:
              protocol: anthropic
    ...
    it "raises naming the deployment when 'model' is missing" do
      expect_raises(Cogiteer::ConfigError, /"haiku".*model/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            anthropic:
              protocol: anthropic
    ...
    it "raises on an unrecognised protocol" do
      expect_raises(Cogiteer::ConfigError, /unrecognised protocol "azure-native"/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            weird:
    ...
    it "raises on an unrecognised max_tokens_field" do
      expect_raises(Cogiteer::ConfigError, /unrecognised max_tokens_field "output_limit"/) do
        Cogiteer::Config.from_yaml(<<-YAML)
          servers:
            weird:
              protocol: chat_completions
  ...
  describe "model preferences" do
    ...
    it "explains itself when YAML turns a bare off into a boolean" do
      expect_raises(Cogiteer::ConfigError, /boolean.*write 'none'/) do
        deployment_with("reasoning: off")
      end
    end

    it "rejects a non-positive budget" do
      expect_raises(Cogiteer::ConfigError, /must be positive/) { deployment_with("reasoning: 0") }
    end

    it "rejects an unrecognised level" do
      expect_raises(Cogiteer::ConfigError, /unrecognised reasoning "ludicrous"/) do
    ...
    it "rejects an unrecognised retention mode" do
      expect_raises(Cogiteer::ConfigError, /unrecognised reasoning_retention "sometimes"/) do
  ...
  describe "#deployment" do
    it "raises listing the known deployment names when asked for an unknown one" do
      ...
      expect_raises(Cogiteer::ConfigError, /"azure-mini".*haiku/) { config.deployment("azure-mini") }
    end
  end
  ...
  describe Cogiteer::ServerConfig do
    it "resolves credential from the named environment variable" do
      ENV["COGITEER_SPEC_TEST_KEY"] = "shh"
      begin
        s = Cogiteer::ServerConfig.new("x", Liaison::ProtocolKind::Anthropic, "https://example.com",
          credential_env: "COGITEER_SPEC_TEST_KEY")
        s.credential.should eq("shh")
      ensure
        ENV.delete("COGITEER_SPEC_TEST_KEY")
      end
    end

    it "is nil when credential_env is unset" do
      s = Cogiteer::ServerConfig.new("x", Liaison::ProtocolKind::ChatCompletions, "http://localhost:11434")
