...
describe Cogiteer::Progress do
  ...
  it "writes nothing at all when the stream is not a terminal" do
    sink = IO::Memory.new
    Cogiteer::Progress.while_waiting("thinking", sink) do
      sleep(Cogiteer::Progress::TICK * 3)
    end
  ...
  it "erases the line even when the block raises" do
    ...
    expect_raises(ArgumentError, "boom") do
      Cogiteer::Progress.while_waiting("thinking", sink) do
        sleep(Cogiteer::Progress::TICK * 2)
  ...
  it "reports elapsed seconds, not just motion" do
    sink = FakeTty.new
    Cogiteer::Progress.while_waiting("thinking", sink) do
      sleep(Cogiteer::Progress::TICK * 5)
    end
  ...
  it "picks up a label changed while running" do
    sink = FakeTty.new
    Cogiteer::Progress.while_waiting("thinking", sink) do |indicator|
      sleep(Cogiteer::Progress::TICK * 2)
  ...
  it "returns the block's value untouched" do
    sink = IO::Memory.new
    result = Cogiteer::Progress.while_waiting("thinking", sink) { {1, "two"} }
    result.should eq({1, "two"})
  end

  it "is safe to stop without having started" do
    Cogiteer::Progress.new("thinking", IO::Memory.new).stop
  end
end
