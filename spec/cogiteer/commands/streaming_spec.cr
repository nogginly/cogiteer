...
# what `docs/DESIGN.md` says it is for and the only way either mode can be
...
private STREAM_START    = "stream_start"
private STREAM_CONTINUE = "stream_continue"
...
# `$COGITEER_HOME` rather than `Dir.cd`.
private def with_sandbox(&) : Nil
  tmp = File.join(Dir.tempdir, "cogiteer-streaming-spec-#{Random.rand(1_000_000)}")
...
private def printed : String
  Cogiteer::Output.stream.to_s
end

private def warned : String
  Cogiteer::Output.error_stream.to_s
end
...
private def saved_reply(id : String) : M::Message
  Cogiteer::Sessions.latest(id).messages.last
end

describe "the CLI's streamed turn" do
  describe "cogiteer start --stream" do
    it "saves the session a non-streamed run would have saved" do
      ...
      with_sandbox do
        Wiretap.intercept(STREAM_START) do
          Cogiteer::Commands::Start.run(["ollama", PROMPT, SYSTEM, "--stream"])
        end

        session = Cogiteer::Sessions.latest(only_session)
  ...
  describe "cogiteer continue --stream" do
    it "appends a streamed turn to a session that already exists" do
      with_sandbox do
        ...
        Wiretap.intercept(STREAM_CONTINUE) do
          Cogiteer::Commands::Continue.run([id, "And the second tallest?", "--stream"])
        end
    ...
    it "prints only the second answer, not the conversation it continued" do
      ...
      with_sandbox do
        ...
        Cogiteer::Output.stream.as(IO::Memory).clear
    ...
    it "declines to stream into a redirected stdout, whatever the config says" do
      ...
      with_sandbox do
        Wiretap.intercept("start_ollama") do
          Cogiteer::Commands::Start.run(["ollama", PROMPT, SYSTEM])
        end
