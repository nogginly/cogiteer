...
module Cogiteer
  ...
  module Sessions
    ...
    # `$COGITEER_HOME` first if set — explicit beats implicit, the same
    # reasoning `Config` now applies to `$LIAISON_CONFIG`. This is a change
    # from the original order (`$CWD`, then `$HOME`, then `$COGITEER_HOME` as
    # a last resort): a real invocation's `.cogiteer` sitting in `$CWD` or
    ...
    def root : String
      return ENV["COGITEER_HOME"] if ENV["COGITEER_HOME"]?

      cwd_candidate = File.join(Dir.current, ".cogiteer")
      return cwd_candidate if Dir.exists?(cwd_candidate)

      if home = ENV["HOME"]?
        return File.join(home, ".cogiteer")
      end

      raise SessionError.new("no $HOME and no $COGITEER_HOME — nowhere to store sessions")
    end
    ...
    def snapshot(id : String, session : Liaison::MPSH::Session, deployment : String) : String
      dir = path_for(id)
      Dir.mkdir_p(dir)
      ...
      File.write(file, Liaison::MPSH::Archive.write(session))
      file
    end
    ...
    def latest(id : String) : Liaison::MPSH::Session
      Liaison::MPSH::Archive.read(latest_archive(id))
    end
