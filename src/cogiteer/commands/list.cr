...
module Cogiteer::Commands
  # `cogiteer list` — what conversations exist, and which deployment each one
  # was last having.
  #
  ...
  module List
    ...
    private def turns(session : Liaison::MPSH::Session) : Int32
      Liaison::MPSH::Turns.segment(session.messages).size
    end
    ...
    private def preview(session : Liaison::MPSH::Session) : String
