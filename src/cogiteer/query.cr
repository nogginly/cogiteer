...
module Cogiteer
  ...
  module Query
    ...
    def run(provider : Liaison::Provider, model : String, session : Liaison::MPSH::Session,
            ...
            indicator : Progress? = nil) : {Liaison::MPSH::Message, Liaison::Capability::Report}
      session << Liaison::MPSH::Message.user(prompt)

      client = Liaison::Client.new(provider)
      options = Liaison::Options.new(reasoning: reasoning)
      ...
      if repaired = Liaison::MPSH::Repair.repaired(reply)
        session << repaired
      end
