...
module Cogiteer
  # The reply is the only thing on stdout, so `cogiteer start ... | pbcopy`
  ...
  module Output
    ...
    def reply(message : Liaison::MPSH::Message) : Nil
    ...
    def warn_cut(reply : Liaison::MPSH::Message) : Nil
      return if reply.ending.complete?

      dropped = reply.content.count { |block| block.is_a?(Liaison::MPSH::ToolCallBlock) }
      note = case reply.ending
             in Liaison::MPSH::Ending::Complete    then return
             in Liaison::MPSH::Ending::Truncated   then "the model stopped short — an output cap or a resource limit"
             in Liaison::MPSH::Ending::Stopped     then "the turn was stopped"
             in Liaison::MPSH::Ending::Interrupted then "the stream ended before the reply did"
             end
    ...
    # Liaison::Reasoning goes to stderr, always, streamed or not.
    #
    ...
    # what changes that. `cogiteer start … > answer.txt` gets an answer, not an
    ...
    def warn_lossy(report : Liaison::Capability::Report) : Nil
    ...
    def transcript(session : Liaison::MPSH::Session) : Nil
